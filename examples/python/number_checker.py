#!/usr/bin/env python3
"""Temu Number Checker API — asynchronous bulk example."""
import os
import time
import requests
...
def submit_task(file_path):
    with open(file_path, "rb") as stream:
        response = requests.post(
            ...
            data={"task_type": TASK_TYPE}, timeout=30)
    response.raise_for_status()
    return response.json()

def get_task(task_id):
    response = requests.post(
        f"{BASE_URL}/v1/gettasks", headers={"X-API-Key": API_KEY},
        data={"task_id": task_id}, timeout=30)
    response.raise_for_status()
    return response.json()
...
def poll(task_id, interval=5):
    while True:
        ...
        if task.get("status") == "exported": return task
        if task.get("status") == "failed": raise RuntimeError("task failed")
        time.sleep(interval)
...
def download(url, output="results.zip"):
    ...
    with open(output, "wb") as stream:
        for chunk in response.iter_content(chunk_size=8192): stream.write(chunk)
    return output
...
if __name__ == "__main__":
    ...
    if final.get("result_url"): print("saved to:", download(final["result_url"]))
