// Temu Number Checker API — Node.js 18+ example. Docs: https://docs.numberchecker.ai/temu-checker
...
if (!API_KEY) {
  throw new Error("NUMBERCHECKER_API_KEY is required");
}
const TASK_TYPE = "temu";

async function submitTask(path) {
  const form = new FormData();
  form.append("file", new Blob([fs.readFileSync(path)]), "numbers.txt");
...
const created = await submitTask("examples/numbers.txt");
...
for (;;) {
  ...
  if (task.status === "exported") {
    if (task.result_url) {
      await downloadResult(task.result_url);
      console.log("saved to: results.zip");
    }
    break;
  }
